...
**QuantumX** should run on any flavour of unix.  I have run it
...
Package          Notes
================ ========================
Python3          QuantumX uses asyncio.  Python version >= 3.7 is
                 **required**.
...
You **must** to be running a non-pruning pdccpon daemon with::

  txindex=1

set in its configuration file.  If you have an existing installation
of pandorad and have not previously set this you will need to reindex
the blockchain with::
...
While not a requirement for running QuantumX, it is intended to be
...
When building the database from the genesis block, QuantumX has to
...
HDD.  Currently to around height 611,600 of the pdccpon blockchain the
final size of the leveldb database, and other QuantumX file metadata
...
+ `plyvel <https://plyvel.readthedocs.io/en/latest/installation.html>`_ for LevelDB.
  ...
  Included as part of a regular pip or ``setup.py`` installation of QuantumX.
+ `python-rocksdb <https://pypi.python.org/pypi/python-rocksdb>`_ for RocksDB

  ``pip3 install python-rocksdb`` or use the rocksdb extra install option to QuantumX.
...
Check out the code from Github::
    ...
    git clone https://github.com/johiruddinsultan/quantumx.git
    cd quantumx
...
under one account which I have called ``quantumx``.

Next create a directory where the database will be stored and make it
writeable by the ``quantumx`` account.  I recommend this directory
live on an SSD::

    mkdir /path/to/db_directory
    chown quantumx /path/to/db_directory


Process limits
--------------

You must ensure the QuantumX process has a large open file limit.
...
Note that setting the limit in your shell does *NOT* affect QuantumX
unless you are invoking QuantumX directly from your shell.  If you
are using :command:`systemd`, you need to set it in the
:file:`.service` file (see `contrib/systemd/quantumx.service`_).
...
logging process.  You can run :command:`svscan` under the *quantumx*
...
of::
    ...
    mkdir ~/service      # As quantumx if running svscan as that a/c
...
Suppose this directory is called :file:`scripts`, you might do::
    ...
    mkdir -p ~/scripts/quantumx

Then copy the all sample scripts from the QuantumX source tree there::

    cp -R /path/to/repo/quantumx/contrib/daemontools ~/scripts/quantumx
...
under :file:`env/` to your setup.  QuantumX server currently takes no
...
svscan is now waiting for services to be added to the directory::
    ...
    cd ~/service
    ln -s ~/scripts/quantumx quantumx
...
to setup QuantumX with systemd. Simply copy it to
:file:`/etc/systemd/system`::

    cp contrib/systemd/quantumx.service /etc/systemd/system/

The sample unit file assumes that the repository is located at
:file:`/home/quantumx/quantumx`. If that differs on your system, you
need to change the unit file accordingly.

You need to set a few :ref:`environment variables <environment>` in
:file:`/etc/quantumx.conf`.

Now you can start QuantumX using :command:`systemctl`::

    systemctl start quantumx

You can use :command:`journalctl` to check the log output::

    journalctl -u quantumx -f

Once configured you may want to start QuantumX at boot::

    systemctl enable quantumx

.. Warning:: systemd is aggressive in forcibly shutting down
   processes.  Depending on your hardware, QuantumX can need several
...
`contrib/raspberrypi3/install_quantumx.sh`_.

See also `contrib/raspberrypi3/run_quantumx.sh`_ for an easy way to
configure and launch quantumx.
...
busy.  QuantumX uses Python's :mod:`asyncio` to prefill a cache of
...
size QuantumX uses; see :ref:`here <CACHE>` for caveats.
...
blocks over the LAN from a pandorad on machine B.  :envvar:`DB_CACHE`
the default of 1,200.  LevelDB.

*Machine B*: a late 2012 iMac running Sierra 10.12.2, 2.9GHz quad-core
Intel i5 CPU with an HDD and 24GB RAM.  Running pandorad on the same
machine.  :envvar:`DB_CACHE` set to 1,800.  LevelDB.

For chains other than pdccpon-mainnet synchronization should be much
faster.

.. note:: QuantumX will not serve normal client connections until it
          has fully synchronized and caught up with your daemon.
          However LocalRPC connections are served at all times.


Terminating QuantumX
=====================
...
like so::
    ...
    svc -d ~/service/quantumx

QuantumX will note receipt of the signals in the logs, and ensure the
...
QuantumX uses the transaction functionality, with fsync enabled, of
...
if the QuantumX process if forcibly killed or there is loss of power.
...
Here is typical log output on startup::
  ...
  INFO:BlockProcessor:software version: QuantumX 0.10.2
  INFO:BlockProcessor:DB version: 5
  INFO:BlockProcessor:coin: pdccpon
...
.. _`contrib/systemd/quantumx.service`: https://github.com/spesmilo/quantumx/blob/master/contrib/systemd/quantumx.service
...
.. _`contrib/raspberrypi3/install_quantumx.sh`: https://github.com/spesmilo/quantumx/blob/master/contrib/raspberrypi3/install_quantumx.sh
.. _`contrib/raspberrypi3/run_quantumx.sh`: https://github.com/spesmilo/quantumx/blob/master/contrib/raspberrypi3/run_quantumx.sh

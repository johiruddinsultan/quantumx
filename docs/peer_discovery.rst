...
This was implemented in QuantumX as of version 0.11.0.  Support for
IRC peer discovery was removed in QuantumX version 1.2.1.
...
In QuantumX this is a per-coin property in `lib/coins.py
<https://github.com/spesmilo/quantumx/blob/master/quantumx/lib/coins.py>`_.


server.peers.subscribe
----------------------

:func:`server.peers.subscribe` is used by Quantum clients to get a
...
In QuantumX, "recently" is taken to be the last 24 hours.  Only one
...
Quantum server should make another attempt to connect, choosing
...
QuantumX will connect to the SSL port if both ports are available.
...
QuantumX attempts to connect to onion peers through a Tor proxy that
...
use by Quantum clients as well as other peers.  Peers will use it to

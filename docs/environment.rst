...
QuantumX takes no command line arguments, instead its behaviour is controlled by
...
.. envvar:: COIN
  ...
  `lib/coins.py`_. For example, ``Pandora``.
...
.. envvar:: DAEMON_URL
  ...
  QuantumX will initially connect to the first, and failover to
...
.. envvar:: QUANTUMX
  ...
  The path to the quantumx_server script.  Relative paths should
...
.. note:: QuantumX will not serve any incoming connections until it has fully caught up
          with your Pandora daemon.  The only exception is local **RPC** connections,
          which are served at any time after the server has initialized.

.. envvar:: SERVICES

  A comma-separated list of services QuantumX will accept incoming connections for.
  ...
  QuantumX will not listen for any incoming connections.
  ...
  *port* can only be defaulted for **rpc** where the default is :const:`8000`.
  ...
  libcap2-bin package::
    ...
    sudo setcap cap_net_bind_service=+ep /path/to/quantumx_server
...
.. envvar:: REPORT_SERVICES
  ...
  A comma-separated list of services QuantumX will advertize and other servers in the
...
.. envvar:: BANNER_FILE
  ...
  + ``$SERVER_VERSION`` is replaced with the QuantumX version you are
    running, such as ``1.0.10``.
  + ``$SERVER_SUBVERSION`` is replaced with the QuantumX user agent
    string.  For example, ``QuantumX 1.0.10``.
  ...
  + ``$DAEMON_SUBVERSION`` is replaced with the daemon's user agent
    string.  For example, ``/PandoraUnlimited:0.12.1(EB16; AD4)/``.
  + ``$DONATION_ADDRESS`` is replaced with the address from the
    :envvar:`DONATION_ADDRESS` environment variable.

  See `here <https://github.com/shsmith/quantumx-banner-updater>`_
...
.. envvar:: REORG_LIMIT
  ...
  reorganisation.  QuantumX retains some fairly compact undo
  information for this many blocks in levelDB.  The default is a
  function of :envvar:`COIN` and :envvar:`NET`; for Pandora mainnet it
  is 200.
...
Address subscriptions in QuantumX are very cheap - they consume about
...
.. envvar:: MAX_RECV
  ...
  Note that the smallest sane/safe value for Pandora is ~800,100 bytes,
...
.. envvar:: MAX_SEND
  ...
  Pandora mainnet are over 500KB in size.
...
.. envvar:: SESSION_GROUP_BY_SUBNET_IPV6
  ...
  to process it.  Factors include whether the request uses Pandorad, how much bandwidth
  ...
  If a session disconnects, QuantumX continues to associate its cost with its IP address,
...
In response to the :func:`server.peers.subscribe` RPC call, QuantumX
will only return peer servers that it has recently connected to and
verified basic functionality.

If you are not running a Tor proxy QuantumX will be unable to connect
...
to be running a Tor proxy for QuantumX to use.

QuantumX will perform peer-discovery by default and announce itself
...
.. envvar:: PEER_DISCOVERY
  ...
  If ``on``, QuantumX will occasionally connect to and verify its
...
.. envvar:: PEER_ANNOUNCE
  ...
  If not defined, or non-empty, QuantumX will announce itself to
  peers.

  If peer discovery is disabled this environment variable has no
  effect, because QuantumX only announces itself to peers when doing
...
.. envvar:: TOR_PROXY_PORT
  ...
  The port on which the Tor proxy is running.  If not set, QuantumX
...
.. _lib/coins.py: https://github.com/spesmilo/quantumx/blob/master/quantumx/lib/coins.py

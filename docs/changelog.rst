...
Note: this is the first release since forking from kyuupichan/quantumx.
kyuupichan has the :code:`quantumx` name on PyPI, so we needed a new name there.
...
* security: a vulnerability has been fixed that allowed a remote attacker to
  crash quantumx if peer discovery was enabled (`#22`_)
...
* mark 4 old environment variables obsolete.  QuantumX won't start until they are removed
...
* quantumx_rpc: timeout raised to 30s, fix session request counts

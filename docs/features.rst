...
- Daemon failover.  More than one daemon can be specified, and
  QuantumX will failover round-robin style if the current one fails
  for any reason.
...
QuantumX does not do any pruning or throwing away of history.  I want
...
- more compact and efficient representation of UTXOs, address index,
  ...
  contrast QuantumX just stores the transaction number in the linear
  history of transactions.  QuantumX can determine block
  height from a simple binary search of tx counts stored on disk.
  QuantumX stores historical transaction hashes in a linear array on
  disk.
...
- leveraging asyncio for asynchronous prefetch of blocks to mostly
  eliminate CPU idling.  As a Python program QuantumX is unavoidably
  single-threaded in its essence; we must keep that CPU core busy.

Python's :mod:`asyncio` means QuantumX has no (direct) use for threads

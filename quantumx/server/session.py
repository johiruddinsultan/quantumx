...
from quantumx.lib.hash import (HASHX_LEN, Base58Error, hash_to_hex_str,
                                hex_str_to_hash, sha256)
from quantumx.lib.merkle import MerkleCache
from quantumx.lib.text import sessions_lines
from quantumx.server.daemon import DaemonError
from quantumx.server.peers import PeerManager

if TYPE_CHECKING:
    from quantumx.server.db import DB
    from quantumx.server.env import Env
    from quantumx.server.block_processor import BlockProcessor
    from quantumx.server.daemon import Daemon
    from quantumx.server.mempool import MemPool
...
class SessionManager:
    ...
    def __init__(
        ...
        self._sslc = None
        # Event triggered when quantumx is listening for incoming requests.
    ...
    async def rpc_add_peer(self, real_name):
        ...
        real_name: "bch.quantumx.cash t50501 s50502" for example
        '''
...
class SessionBase(RPCSession):
    '''Base class of QuantumX JSON sessions.
    ...
    async def handle_request(self, request):
        '''Handle an incoming request.  QuantumX doesn't receive
...
class QuantumX(SessionBase):
    ...
    def server_features(cls, env):
        ...
        return {
            'hosts': hosts_dict,
            'pruning': None,
            'server_version': quantumx.version,
    ...
    def server_version_args(cls):
        ...
        return [quantumx.version, cls.protocol_min_max_strings()]
    ...
    async def replaced_banner(self, banner):
        ...
        for pair in [
                ('$SERVER_VERSION', quantumx.version_short),
                ('$SERVER_SUBVERSION', quantumx.version),
    ...
    async def banner(self):
        ...
        banner = f'You are connected to an {quantumx.version} server.'
        self.bump_cost(0.5)
    ...
    async def server_version(self, client_name='', protocol_version=None):
        ...
        return quantumx.version, self.protocol_version_string()
...
class SmartCashQuantumX(DashQuantumX):

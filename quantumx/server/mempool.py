...
from quantumx.lib.hash import hash_to_hex_str, hex_str_to_hash
from quantumx.lib.util import class_logger, chunks
from quantumx.server.db import UTXO
...
class MemPoolAPI(ABC):
    ...
    async def height(self):
        '''Query pandorad for its height.'''

    @abstractmethod
    def cached_height(self):
        '''Return the height of pandorad the last time it was queried,
    ...
    async def mempool_hashes(self):
        '''Query pandorad for the hashes of all transactions in its
        mempool, returned as a list.'''

    @abstractmethod
    async def raw_transactions(self, hex_hashes):
        '''Query pandorad for the serialized raw transactions with the given
